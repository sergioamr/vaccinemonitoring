...
void sms_send_heart_beat() {
	...
	//current: SENSORS, BATTERY, CHARGING_STATUS, UPTIME
	/*
	...
	sprintf(msg, SMS_HB_MSG_TYPE "%s,%d,%s,%s,%s%d,%d,%d,%d,%d", g_pDevCfg->cfgIMEI,
			config_getSelectedSIM(), g_pDevCfg->cfgGatewaySMS,
			sim->cfgSMSCenter, sensors, batt_getlevel(), !(P4IN & BIT4), g_pSysState->simState[slot].failedTransmissionsGPRS,
			g_pSysState->simState[slot].failedTransmissionsGSM, s->alarms.SD_card_failure);
