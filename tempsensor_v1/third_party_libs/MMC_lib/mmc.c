...
char mmcInit(void)
{
  ...
  //ZZZZ disabled as it GPIO configuration will be done before
  //disabling the GPIO power-on default high-impedance
...
char mmcGoIdle()
{
  ...
  while(1)
  {
    ...
    if(iIdx < 64)
    {
    	mmcSendCmd(MMC_GO_IDLE_STATE,0,0x95);	//retry to go to idle state
    }
    else
    {
    	ret = MMC_INIT_ERROR;
...
char mmcGetResponse(void)
{
  ...
  char response = 0x7e;		//initialize the response code

  while(i<=64)				//ZZZ recheck the limit
