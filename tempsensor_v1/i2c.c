...
void i2c_read(uint8_t ucSlaveAddr, uint8_t ucCmd, uint8_t ucLen,
		uint8_t* pucData) {
	...
	memcpy(pucData, (const void *) I2CRX, ucLen);
}

void i2c_write(uint8_t ucSlaveAddr, uint8_t ucCmd, uint8_t ucLen,
		uint8_t* pucData) {
	UCB0I2CSA = ucSlaveAddr;                  				// Slave address
	cMode = I2C_WRITE;
	I2CTX[0] = ucCmd;
	memcpy((void *) &I2CTX[1], pucData, ucLen);
	iI2CTxIdx = 0;
	iI2CTxLen = ucLen + 1;
